...
#[test]
#[ignore = "requires a running RustFS server and an existing bucket"]
fn rustfs_copy_verify_restore_and_move_roundtrip() -> Result<()> {
    if env::var("DELTABOX_RUN_RUSTFS_TESTS").as_deref() != Ok("1") {
        eprintln!("skipping RustFS integration test; set DELTABOX_RUN_RUSTFS_TESTS=1");
        return Ok(());
    }
    ...
    let access_key = env::var("DELTABOX_S3_ACCESS_KEY").unwrap_or_else(|_| "rustfsadmin".to_owned());
    let secret_key = env::var("DELTABOX_S3_SECRET_KEY").unwrap_or_else(|_| "rustfsadmin".to_owned());
    let prefix = env::var("DELTABOX_S3_PREFIX")
        .ok()
        .unwrap_or_else(|| format!("integration/{}", Uuid::new_v4()));

    let root = env::temp_dir().join(format!("deltabox-rustfs-test-{}", Uuid::new_v4()));
    ...
    while input_bytes.len() < 1_200_000 {
        input_bytes.extend_from_slice(b"deltabox rustfs integration payload\n");
    }
    ...
    let copied = vault.copy_file_to_backend(&manifest.file_id, "rustfs")?;
    assert!(copied.chunks.iter().all(|chunk| {
        chunk
            .locations
            .iter()
            .any(|location| location.backend_id == "rustfs")
    }));
    ...
    assert!(s3_only_locations
        .iter()
        .all(|location| location.backend_id == "rustfs"));

    let restored_from_rustfs =
        vault.restore_file(&manifest.file_id, output_dir.join("from-rustfs.txt"))?;
    assert_eq!(fs::read(restored_from_rustfs)?, input_bytes);
