...
邮箱后端借鉴 Delta Chat 的思想：复用已有邮箱基础设施，而不是依赖 deltabox 官方服务器。邮箱在 deltabox 中承担两个角色。
...
- 几乎每个用户都有邮箱，不需要注册新服务
- IMAP/SMTP 是开放协议，服务商可替换
- 邮件天然是追加式日志，投递可靠、自带时间戳和唯一 ID
- 多设备共用一个邮箱即构成 vault 的最终一致性通道

**角色二：小数据存储后端**

- chunk 作为 MIME 附件保存，使用专门文件夹，例如 `DeltaBox/Chunks`
- SMTP 负责写入，IMAP 负责读取
- 适合小文件、低频备份或重要文件的冗余副本
- 大文件内容不走邮箱，事件和 manifest 中只携带其在其他后端的位置指针
...
- 邮箱附件大小有限（普遍 10-50MB），不适合照片/视频的主存储
- 服务商可能限流（每日发送配额），批量自动化发信可能触发反垃圾机制
- IMAP 轮询有延迟，同步是秒级到分钟级
- 邮箱存储配额有限（且常与个人邮件共享配额）
- 定位为同步通道 + 小数据冗余，不作为唯一主存储
...
邮箱
  事件以加密邮件写入 DeltaBox/Events 文件夹（见 6.6），
  设备通过 IMAP 轮询拉取新邮件。
...
邮箱适合去中心化和普及性，但不适合作为唯一主存储。附件大小、发送限流、配额和拉取延迟决定了邮箱更适合承担两类负载：小型 chunk 的低频冗余备份，以及事件日志 / manifest 这类小体积、追加式的同步流量。大文件内容应放对象存储，邮箱通道只携带位置指针。此外，事件邮件必须先有客户端加密，否则邮箱服务商可以看到完整的文件清单。
...
- 邮箱后端优先支持哪些服务商？哪些服务商的 IMAP 轮询和限流适合承载事件同步流量？
