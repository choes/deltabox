...
- DOCX 正文文本索引，支持 `word/document.xml` 中的段落和表格文本
- DOCX 页眉页脚文本索引，使用 `docx_header_footer` 来源标记
- XLSX 单元格文本索引，支持 shared strings、inline strings 和普通单元格值
- PPTX 幻灯片文本索引，支持按 slide 写入 `text_segments`
- PPTX speaker notes 索引，使用 `pptx_speaker_notes` 来源标记
- 图片元数据索引，支持 JPEG / PNG / GIF 的格式、尺寸、文件大小、EXIF 设备/方向/拍摄时间和 GPS 坐标
- 视频基础元数据索引，支持 MP4 / MOV 的时长、创建时间、尺寸和 codec
- Office 文档索引容错：损坏文件不阻塞导入，并记录索引错误
...
- DOCX body text indexing from `word/document.xml` paragraphs and table text
- DOCX header/footer text indexing with `docx_header_footer` source markers
- XLSX cell text indexing from shared strings, inline strings, and plain cell values
- PPTX slide text indexing with slide-level `text_segments`
- PPTX speaker notes indexing with `pptx_speaker_notes` source markers
- Image metadata indexing for JPEG / PNG / GIF format, dimensions, file size, EXIF device/orientation/capture time, and GPS coordinates
- Basic video metadata indexing for MP4 / MOV duration, creation time, dimensions, and codec
- Office indexing fault tolerance so corrupt files do not block import and record index errors
